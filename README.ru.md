...
- **Никакой модели, никакого обучения, никакого инференса.** Демон - это один файл на
  Python из стандартной библиотеки: он читает датчики, сравнивает числа с порогами и
  посылает `SIGSTOP` / `SIGCONT`. Журнал событий (`~/.coffee-paladin/history_events.jsonl`) -
  это простая история того, что произошло; на нём ничего не обучается. Единственное число,
  которое guard считает наперёд, `eta_pause_min`, - это линейная экстраполяция температуры
  чипа за последние пять минут, хранящейся в памяти; сам журнал при этом никогда не читается.
- **Никакого открытого порта.** `thermal-metrics` пишет текст в формате Prometheus для
  textfile-коллектора `node_exporter`; ничто в проекте не слушает ни на одном сокете.
- **Один опциональный исходящий канал в демоне.** Пуши на телефон идут через ntfy.sh, и только
  если вы задали `ntfy_topic` в `config.json`. По умолчанию поле пустое, и тогда демон
  вообще не делает сетевых запросов. (`install.sh` один раз использует Homebrew, чтобы
  поставить `macmon`.) Исключение - панель меню: если в системе установлен внешний бинарник
  `ccusage`, панель запускает его каждые десять минут ради строки расхода, а `ccusage`
  тянет цены моделей из сети, если не запустить его с `--offline` - а панель этот флаг
  не передаёт.
- **Сам не является датчиком.** Показания чипа, GPU, вентиляторов и питания приходят от
  [`macmon`](https://github.com/vladkens/macmon) - внешнего бинарника с открытым кодом,
  который без `sudo` читает интерфейсы датчиков Apple (SMC для температур и вентиляторов,
  IOReport для питания); guard разбирает его вывод, а без него откатывается на температуру
  батареи.
- **Не привязан ни к одному AI-инструменту.** Skill для агентов, хуки, строка статуса и
  манифест плагина - опциональные интеграции. `install.sh` ставит skill и подключает строку
  статуса только туда, где нужный инструмент уже стоит на машине, и никогда не подменяет
  чужую строку статуса, если не передать `--replace`. Ни один хук он не подключает вовсе:
  адаптеры хуков копируются в `~/.coffee-paladin/`, а включается каждый вручную (см. раздел
  про хуки ниже). Демон работает и без всего этого.
...
Всё в этом разделе - опционально. Демон защищает машину и без этого; здесь описаны
способы, какими другие программы на Mac могут *спросить* guard, прежде чем начать
его греть. Сегодня это в первую очередь терминалы и CLI-агенты кода - именно они
запускают сборки, кодирование и инференс моделей на ноутбуке, не глядя на вентилятор.
...
«Активность ИИ-агентов» появляется строка сегодняшнего расхода по всем CLI
...
**Активность ИИ-агентов.** Демон пишет `agent_activity.json`: какие сессии AI
работают на этом Mac и какое дерево процессов каждая запустила, с тепловым
контекстом. В меню это подменю «Активность ИИ-агентов» с иконками по типу
процесса, а на панели значок ✨ горит, пока жива хоть одна сессия - само
присутствие значка отвечает на вопрос «работает ли сейчас AI».
...
**Также в репозитории, чтобы никто не наткнулся на это случайно:**

- `.claude-plugin/plugin.json` - манифест плагина для Claude Code, который упаковывает
  agent skill (`skills/coffee-paladin/`) как плагин. Guard он не устанавливает и не
  запускает - демон по-прежнему ставится через `install.sh` или Homebrew. Это ещё один
  опциональный способ получить skill, не другой продукт.
- В панели меню есть строка *Лимиты Claude: …*. Она существует, только пока интеграция
  со строкой статуса Claude Code пишет `~/.coffee-paladin/claude_usage_cache.json`
  (отфильтрованные поля, без ID сессий и путей); когда файла нет или он старше пяти
  минут, функция ничего не возвращает и строка не рисуется. Панель никогда сама не
  запрашивает данные аккаунта.
- В `integrations/` лежат хуки для пяти CLI-агентов (Claude Code, Codex, Gemini CLI,
  Grok, Antigravity) рядом с интеграциями терминалов. Хук заставляет агента спросить
  guard перед тяжёлой работой; без хука guard всё равно ставит эту работу на паузу,
  когда чип нагревается.
...
- Температура чипа берётся через `macmon` (SMC, без sudo). Без него guard работает,
...
- Только **Apple Silicon** и **macOS 14 или новее**. На Intel-маках путь к датчикам
  совсем другой, и он пока не реализован.
...
Автор: Paweł Kwaczyński / FOCUS FRAME, 2026. Проект основателя студенческого научного
кружка **AIrON** (информатика, AHE, Лодзь).
