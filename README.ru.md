...
Без `sudo`. Без расширений ядра. Без демонов от root. Все датчики читаются
обычным пользовательским процессом.
...
**5. Калибрует себя под машину.** При первом запуске измеряет, что это за Mac —
есть ли вентиляторы, сколько ядер, какой чип — и выбирает пороги под него.
Безвентиляторные машины получают более низкие пороги и более длинную допустимую паузу.

**6. Хранит доказательства (чёрный ящик).** Каждый цикл пишет пульс, а после перезагрузки
сравнивает его с временем старта: если последний пульс позже момента загрузки, машина
выключилась жёстко. Рядом лежит история температур. `thermal-report` собирает из этого
один документ, который примут в сервисе.
...
приостанавливает задачи, а кто молчит уже несколько часов.
...
Он учит четырём вещам: сначала прочитать `~/.thermal-guard/status.json`
...
не трогает. Когда убедитесь, что видит он разумные вещи, снимите галочку
*Настройки → Только наблюдение* — защита включится. Вернуть обратно можно в любой момент,
перезапуск не нужен.
...
safe-run -- ffmpeg -i in.mp4  # правильный способ запустить тяжёлую задачу
thermal-report                # отчёт для сервисного центра
...
польском, русском, китайском и испанском. Переключение — *Настройки → Язык*.
...
- Только **Apple Silicon**. На Intel-маках путь к датчикам совсем другой (SMC вместо
  IOReport), и он пока не реализован.
...
**Графика.** Паладин-маскот **сгенерирован в ChatGPT (OpenAI)** и используется как
