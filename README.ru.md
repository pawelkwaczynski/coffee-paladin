...
- **Никакой модели, никакого обучения, никакого инференса.** Демон - это один файл на
  ...
  это простая история того, что произошло; никакой предиктор на нём не обучается, и его
  не существует.
- **Никакого открытого порта.** `thermal-metrics` пишет текст в формате Prometheus для
  textfile-коллектора `node_exporter`; ничто в проекте не открывает сокет.
- **Один опциональный исходящий канал.** Пуши на телефон идут через ntfy.sh, и только
...
- **Сам не является датчиком.** Показания чипа, GPU, вентиляторов и питания приходят от
  ...
  который читает IOReport; guard разбирает его вывод, а без него откатывается на
  температуру батареи.
- **Не привязан ни к одному AI-инструменту.** Skill для агентов, хуки, строка статуса и
  манифест плагина - опциональные интеграции. `install.sh` кладёт каждую из них только
  туда, где уже стоит соответствующий инструмент, и никогда не подменяет чужую строку
  статуса; демон работает и без всего этого.
...
- `.claude-plugin/plugin.json` - манифест плагина для Claude Code, который упаковывает
  того же guard и skill как плагин. Это ещё один опциональный канал распространения,
  не другой продукт.
- В панели меню есть строка *Claude limits: …*. Она существует, только пока интеграция
