...
limit = 45 * 60
mono = time.monotonic()
for description, entry, expected in [
    ...
    test(description, (pause_duration(entry) > limit) is expected,
         "duration=%.0f s" % pause_duration(entry))
...
for hw in ({"model_id": "Mac15,12", "chip": "M3", "fan_count": 0, "chip_sensor": False},
    ...
    tags.add("%s|%s|fans=%s|sensor=%s" % (hw["model_id"], hw["chip"],
                                          hw["fan_count"], bool(hw["chip_sensor"])))
test("Air before macmon and after macmon has DIFFERENT calibration markers", len(tags) == 2,
     "markers: %s" % tags)
...
saved = dict(g._soc_cache)
real_run, real_mono = g.run, time.monotonic
readings = [0]
...
try:
    ...
    test("right after daemon start, guard really asks chip sensor",
         readings[0] == 1, "macmon reads: %d (0 = guard decided there is no sensor)"
         % readings[0])
finally:
    g.run, g.time.monotonic = real_run, real_mono
    g._soc_cache.clear()
    g._soc_cache.update(saved)
...
for description, entry, expected in [
    ...
    m = entry.get("since_mono")
    d = (max(0.0, time.monotonic() - m) if m is not None and entry.get("mono_id") == g._MONO_ID
         else max(0.0, g.now() - entry.get("since", g.now())))
    test(description, (d > limit) is expected, "duration=%.0f s" % d)
