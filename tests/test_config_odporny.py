...
for key, junk in [("cpu_min_percent", "dwadziescia"), ("soc_pause_c", None),
                     ("poll_seconds", []), ("job_cpu_percent", {"a": 1}),
                     ("notify", "tak"), ("never_extra", "ffmpeg")]:
    cfg = set_config({key: junk})
    try:
        targets = g.pick_targets(cfg, PROCS, {})
        test("bad type in %r does not crash target selection" % key, True)
    except Exception as e:
        test("bad type in %r does not crash target selection" % key, False,
             "%s: %s" % (type(e).__name__, e))

cfg = set_config({"cpu_min_percent": "dwadziescia"})
...
for key in ("never_extra", "never_patterns", "never_arg_patterns"):
    cfg = set_config({key: [""]})
    targets = g.pick_targets(cfg, PROCS, {})
    test("empty string in %s does not remove all candidates" % key,
         len(targets) == 2, "candidates: %d" % len(targets))

cfg = set_config({"never_extra": ["", "   ", None, 7, "blender"]})
targets = g.pick_targets(cfg, PROCS, {})
test("junk in list filtered out, real entry WORKS",
     len(targets) == 1 and targets[0][2] == "ffmpeg",
     "candidates: %s" % [c[2] for c in targets])

# --- 3. The guard's own process names are always protected ---
cfg = set_config({"never_patterns": ["cokolwiek"]})
...
cfg = set_config({"klucz_z_przyszlosci": {"a": [1, 2]}})
...
cfg = set_config({"soc_resume_c": 95, "soc_pause_c": 85})
...
cfg = set_config({"soc_pause_c": 85, "soc_kill_c": 80})
...
cfg = set_config({"soc_resume_c": 70, "soc_pause_c": 85, "soc_kill_c": 92})
...
patterns = [w.lower() for w in g.load_cfg()["never_arg_patterns"]]
...
def protected(cmd):
    ...
    try:
        ...
        g.full_args = old
    return any(w in args for w in patterns)


for description, cmd, oczekiwane in [
    ...
    test(description, protected(cmd) is oczekiwane, "cmd: %s" % cmd[:60])
