...
let PL: [String: String] = [
    ...
    "data is stale (%@) - the guard may have died": "dane nieświeże (%@) - strażnik mógł paść",
    ...
    "Flame at critical": "Płomień przy stanie krytycznym",
    ...
    "resume at %.0f °C, terminate at %.0f °C": "wznowienie przy %.0f °C, zamknięcie przy %.0f °C",
    "continuous watch - the guard itself burns several percent of one core, all day": "ciągły nadzór - strażnik sam zjada kilka procent rdzenia, cały czas",
    "fastest sensible reaction - about 3.5% of one core": "najszybsza sensowna reakcja - około 3,5% rdzenia",
    "default: good reaction at about 1.2% of one core": "domyślnie: dobra reakcja przy około 1,2% rdzenia",
    "very frugal - between two readings a job can go from warm to the kill threshold unnoticed": "bardzo oszczędnie - między dwoma odczytami zadanie może przejść od ciepła do progu ubicia niezauważone",
    "the panel will describe this setting once the guard has watched this Mac for a while": "panel opisze to ustawienie, gdy strażnik pozna już tego Maca",
    "effectively off - in %d readings this Mac never reached %.0f °C, so the pause may never come": "praktycznie wyłączone - w %d odczytach ten Mac nigdy nie osiągnął %.0f °C, więc pauza może nie przyjść nigdy",
    "very late - only %.1f%% of this Mac's readings reach here, so the pause will be rare": "bardzo późno - tylko %.1f%% odczytów tego Maca tu sięga, więc pauza będzie rzadka",
    "balanced - %.0f%% of this Mac's readings reach here, hot but not yet dangerous": "wyważone - %.0f%% odczytów tego Maca tu sięga, gorąco, ale jeszcze nie groźne",
    "early - %.0f%% of this Mac's readings reach here, so ordinary work will be paused": "wcześnie - %.0f%% odczytów tego Maca tu sięga, więc zwykła praca będzie wstrzymywana",
    "Battery gate": "Wstrzymuj na baterii poniżej",
    "pause below this charge when unplugged": "bez zasilacza i poniżej tego poziomu ciężkie zadania poczekają na ładowarkę",
    ...
    "Measurement interval": "Co ile mierzyć",
    ...
    "WHAT YOU CAN SET\n• Chip pause threshold - a slider; resume follows 9 °C lower, gentle closing 5 °C higher.\n• Measurement interval, from one second to five minutes: more often = faster reaction, but costlier in CPU use.\n• Heavy jobs (safe-run): all cores (fast) or efficiency cores only (cool and quiet), plus a CPU limit of 50-100%.\n• Battery gate, signals, keep-awake, this Mac's name in the fleet.": "CO MOŻESZ USTAWIĆ\n• Próg pauzy chipa - suwak; wznowienie idzie 9 °C niżej, łagodne zamknięcie 5 °C wyżej.\n• Co ile mierzyć, od sekundy do pięciu minut: częściej = szybsza reakcja, ale drożej w użyciu CPU.\n• Ciężkie zadania (safe-run): wszystkie rdzenie (szybko) albo tylko E-cores (chłodno i cicho), do tego limit CPU 50-100%.\n• Wstrzymywanie na baterii, sygnały, „Nie usypiaj”, nazwa tego Maca we flocie.",
    ...
    "yes": "tak",
    "no": "nie",
    "Keep the Mac awake while heavy jobs run": "Nie usypiaj Maca przy ciężkich zadaniach",
    ...
    "What the guard did here (total)": "Co strażnik zrobił na tej maszynie (od zawsze)",
...
let RU: [String: String] = [
    ...
    "What the guard did here (total)": "Что страж сделал на этой машине (за всё время)",
    ...
    "data is stale (%@) - the guard may have died": "данные устарели (%@) - страж мог упасть",
    ...
    "Flame at critical": "Пламя при критическом состоянии",
    ...
    "very frugal - between two readings a job can go from warm to the kill threshold unnoticed": "очень экономно - между двумя отсчётами задача может незаметно перейти от тёплого состояния к порогу завершения",
    ...
    "pause below this charge when unplugged": "без адаптера и ниже этого уровня тяжёлые задачи подождут зарядку",
...
""",
    ...
    "Start the guard again": "Запустить стража снова",
    ...
    "Measurement interval": "Интервал измерений",
    ...
    "you have to see this: coffee-paladin": "это нужно увидеть: coffee-paladin",
    "Hey,\n\nI found something you need on your Mac: coffee-paladin. It watches the chip, GPU and battery temperatures, and when things get hot it pauses heavy jobs and resumes them by itself once the machine cools down.\n\nIt is damn good, because the pause is lossless (the process freezes and continues from the same spot), it sends nothing anywhere, and it is free, open source:\n%@\n\nThe knight with the coffee in the attachment is its mascot.\n\nCheers!":
    ...
    "set up: %@": "настроен: %@",
    "Turns off banners, their sounds and phone push - one gate for all.":
        "Отключает уведомления, их звуки и push, один переключатель для всего.",
    "Exception: the critical banner shouts regardless.":
...
let ZH: [String: String] = [
    ...
    "pause below this charge when unplugged": "未接电源且电量低于此值时,繁重任务将等待充电",
    "Notifications": "通知",
...
""",
    ...
    "Turn thermal protection off?": "关闭过热保护?",
    "The paladin will keep measuring and writing down what it would have paused, and will stop nothing at all, even above %.0f °C.": "圣骑士会继续测量并记录它本会暂停的内容,但不会停止任何进程,即使超过 %.0f °C。",
    "Turn protection off": "关闭保护",
    "Keep it on": "保持开启",
    "Watch only": "仅观察",
    "The paladin keeps measuring and writes to the log what it would pause, but it stops nothing until you switch this back on.": "圣骑士继续测量并把它本会暂停的内容写入日志,但在你重新打开保护之前不会停止任何进程。",
    "Thermal protection is on": "过热保护已开启",
    "Above %.0f °C the paladin pauses heavy jobs and starts them again by itself at %.0f °C.": "超过 %.0f °C 时圣骑士会暂停重任务,并在 %.0f °C 时自动恢复。",
    "Jobs resumed": "任务已恢复",
    "Held jobs (%d) are running again; if the chip is still hot the paladin pauses them again on its next reading.": "已暂停的任务(%d 个)又在运行;如果芯片仍然很热,圣骑士会在下一次读数时再次暂停它们。",
...
let ES: [String: String] = [
    ...
    "Throttling: CPU capped at %d%% speed": "Throttling: CPU limitada al %d%% de velocidad",
    ...
    "What is loading the Mac": "Qué está sobrecargando el Mac",
    "Held: %d": "Detenidos: %d",
    "  (by hand)": "  (a mano)",
    "Calm": "En calma",
    ...
    "Flame at critical": "Llama en estado crítico",
    ...
    "pause below this charge when unplugged": "sin adaptador y por debajo de este nivel, las tareas pesadas esperarán al cargador",
